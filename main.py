from dbc import db, app
from models import Grades, Student, Lessons, Logs
from flask import render_template, url_for, redirect, request
from sqlalchemy.exc import IntegrityError

# fonks oluşturup listi sql server'a bağladık html çıktısını aldık
@app.route('/')
...
# öğrenci ekleme fonks ekleniyor ve loglara ekleniyor
...
# ders listeleme fonks oluşturuyoruz
...
# ders ve öğrenci bağlama fonks oluşturuyoruz.
...
def add_student_lesson() -> str:
    ...
    if request.method == 'POST':
        ...
        for check in checklist:
            ...
            log = Logs.create_log('Student_lesson', 'ASL', student[0].name,
                                  lesson[0].lesson_name)
            db.session.add(Logs(log))
        db.session.commit()
        return redirect(url_for('show_student_by_lesson'))
    return render_template("add_student_lesson.html", lesson_list=lesson_list,
                           student_list=student_list)

# ders list görüntüleme
@app.route('/show_lessons')
def show_lessons() -> str:
    lesson_list = db.session.query(Lessons.id, Lessons.lesson_name,
                                   Lessons.teacher_name).all()
    return render_template('lesson_list.html', lesson_list=lesson_list)


# öğrenci ve ders ilişkisi görüntüleme
...
def show_student_by_lesson() -> str:
    ...
    student_list = Student.query.join(Lessons.student).filter(
        Lessons.lesson_name == lesson).all()
    return render_template('student_list_by_lesson.html',
                           student_list=student_list,
                           lesson_list=lesson_list, lesson=lesson)


# öğrenci detayları için fonks oluşturuyoruz
...
def student_details(number) -> str:
    ...
    query = db.session.query(Student, Lessons, Grades).add_columns(
        Lessons.lesson_name,
        Grades.midterm,
        Grades.final).filter(
            Lessons.id == Grades.lesson_id,
            Student.id == Grades.student_id,
            Grades.student_id == student.id).all()
    return render_template('student_details.html', number=number, student=student, query=query)

# öğrencilerin notlarını verip loglama işlemini yapıyoruz
...
def add_notes(number) -> str:
    ...
    if request.method == 'POST':
        ...
        lesson_name = Lessons.query.filter(
            Lessons.id == request.form['lesson_name']).all()[0]
...
#öğrenciyi numarasına göre ilk silip sonra logdan siliyoruz
...
# dersi id sine göre ilk silip sonra loglardan siliyoruz
